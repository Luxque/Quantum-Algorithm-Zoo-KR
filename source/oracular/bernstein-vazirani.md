...
고전 컴퓨터는 $n$개의 쿼리가 필요하다.
번스타인(Bernstein)과 바자라니(Vazarani)가 보인 바와 같이 [[11](/QAZKR/appendix/references.html#11)], 양자 컴퓨터에서는 1개의 쿼리만으로 이를 수행할 수 있다.
더 나아가 이 문제를 재귀적으로 바꾸면 재귀 푸리에(Fourier) 추출 문제가 되는데, 양자 컴퓨터는 고전 컴퓨터보다 지수적으로 적은 쿼리만을 요구한다 [[11](/QAZKR/appendix/references.html#11)].
