...
이 알고리즘의 목표는 $f$의 쿼리를 만들어 $H$(생성자의 집합)를 찾는 것이다.
고전적으로는 $\Omega(|G|)$ 쿼리가 필요하지만, 양자 컴퓨터는 $O(\log|G|)$ 쿼리만으로도 해결할 수 있다.
...
특정한 경우에는 아벨 숨은 부분군 문제는 [[30](/QAZKR/appendix/references.html#30)]에서 보였듯이 $\log(|G|)$ 쿼리가 아닌 단 한개의 쿼리로 해결할 수 있다.
