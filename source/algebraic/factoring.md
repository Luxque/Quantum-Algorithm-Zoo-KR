...
피터 쇼어(Peter Shor)가 고안한 알고리즘은 이를 $O(n^3)$ 시간 내로 찾는다 [[82](/QAZKR/appendix/references.html#82), [125](/QAZKR/appendix/references.html#125)].
고전 알고리즘 중에서 소인수분해를 찾는 가장 빠른 알고리즘은 수체 체로, $2^{\tilde{O}(n^{1/3})}$의 시간 내로 이를 수행한다고 알려져있다.
...
쇼어의 소인수분해 알고리즘은 RSA 암호를 무력화시키며, 이에 깊게 관련된 양자 이산 로그 알고리즘은 DSA/ECDSA 전자 서명 방식과 디피-헬먼 키 교환 프로토콜을 무력화한다.
...
양자 알고리즘으로도 무력화되지 않을 것으로 보이는 고전 공개 키 암호 방식이 발의되어 있다 [[248](/QAZKR/appendix/references.html#248)].
